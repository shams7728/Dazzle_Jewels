...
export default function CartPage() {
    ...
    if (items.length === 0) {
        return (
            <div className="relative min-h-screen bg-background pt-24 pb-12 overflow-hidden">
                ...
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-96 w-96 rounded-full bg-primary/5 blur-3xl pointer-events-none" />
    ...
    return (
        <div className="relative min-h-screen bg-background pt-24 pb-12 overflow-hidden">
            {/* Background Effects */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary/5 via-transparent to-transparent pointer-events-none" />
            <div className="absolute left-1/4 top-20 h-96 w-96 rounded-full bg-primary/5 blur-3xl pointer-events-none" />
            <div className="absolute bottom-20 right-1/4 h-96 w-96 rounded-full bg-pink-400/5 blur-3xl pointer-events-none" />

            <div className="container relative mx-auto px-4">
                {/* Page Header */}
                <header className="mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="flex items-center justify-between mb-4">
                        <h1 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-primary via-pink-400 to-primary bg-clip-text text-transparent drop-shadow-sm">
                            Shopping Cart
                        </h1>
                        <Link
                            href="/products"
                            className="text-sm text-primary hover:text-primary/80 transition-colors flex items-center gap-2 font-medium focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background rounded px-2 py-1"
                            aria-label="Continue shopping for more products"
                        >
                            ← Continue Shopping
                        </Link>
                    </div>
                    <p className="text-muted-foreground text-base" aria-live="polite" aria-atomic="true">
