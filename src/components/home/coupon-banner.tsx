...
export function CouponBanner() {
    ...
    return (
        <div className="relative w-full bg-gradient-to-r from-yellow-500/10 via-yellow-400/5 to-yellow-500/10 border-y border-yellow-500/20 overflow-hidden py-4">
            ...
            <div className="relative flex gap-4 animate-scroll-left hover:pause-animation">
                {duplicatedCoupons.map((coupon, index) => (
                    ...
                    >
                        <div className="relative flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-neutral-900 to-neutral-800 border border-yellow-500/30 rounded-lg shadow-lg hover:shadow-yellow-500/20 transition-all duration-300 cursor-pointer"
                            onClick={() => copyToClipboard(coupon.code)}
                        >
                            {/* Glow effect on hover */}
                            <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/10 to-yellow-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg" />
                            
                            {/* Icon */}
                            <div className="relative flex items-center justify-center w-10 h-10 bg-yellow-500/10 rounded-full border border-yellow-500/30 group-hover:border-yellow-500/60 transition-colors">
                            ...
                            <div className="relative flex flex-col">
                                <div className="flex items-center gap-2">
                                    ...
                                    {copiedCode === coupon.code ? (
                                        ...
                                        <Copy className="h-4 w-4 text-neutral-400 group-hover:text-yellow-500 transition-colors" />
                                    )}
                                </div>
